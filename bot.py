...
from ui import (
    b, i, code, quote, branded, header, divider, progress_bar,
...
pending_dl = {}                   # token -> {"platform": str, "url": str}   (video/audio choice)
awaiting_feature_input = set()    # chat_ids currently expected to reply to a feature prompt

URL_PATTERNS = [
    ("tiktok", re.compile(r"(?:https?://)?(?:vt\.|vm\.|www\.)?tiktok\.com/\S+", re.I)),
    ("youtube", re.compile(r"(?:https?://)?(?:www\.|m\.)?(?:youtube\.com/\S+|youtu\.be/\S+)", re.I)),
    ("instagram", re.compile(r"(?:https?://)?(?:www\.)?instagram\.com/\S+", re.I)),
    ("pinterest", re.compile(r"(?:https?://)?(?:www\.)?(?:pinterest\.[a-z.]+/\S+|pin\.it/\S+)", re.I)),
]


def detect_link(text):
    """Returns (platform, url) for the first recognized link in text, or (None, None)."""
...
def new_pending(platform, url):
    token = uuid.uuid4().hex[:10]
    pending_dl[token] = {"platform": platform, "url": url}
    return token


def extract_audio_ffmpeg(video_bytes):
    """Pulls the audio track out of a video file using ffmpeg. Returns mp3
    bytes, or None if extraction failed (e.g. ffmpeg missing, or the file
...
def ask_video_or_audio(loading, platform, url, label="this"):
    token = new_pending(platform, url)
    markup = with_menu_button([
        success_button("Video", f"dltype:{token}:video"),
        primary_button("Audio", f"dltype:{token}:audio"),
    ])
    finish(loading, branded(header("Ready", "✅") + quote(f"Grab {label} as:")), markup=markup)
...
def main_menu_text(first_name, is_admin_user):
    ...
    return branded(
        ...
        + "\n\n" + quote(
            "🎧 " + b("Quick tip:") + " just paste any TikTok, YouTube, Pinterest, "
            "or Instagram link directly — I'll send you the " + b("audio") + " instantly, no command needed!"
        )
        + "\n\n" + b("👇 Or pick a category to see its commands:") + admin_note
    )
...
def tiktok_download(message):
    ...
    ask_video_or_audio(loading, "tiktok", url, "this TikTok")


def deliver_tiktok(chat_id, url, choice, loading=None):
    feature = "tiktok_audio" if choice == "audio" else "tiktok_download"
    ...
    try:
        ...
        (finish if loading else bot.send_message)(loading or chat_id, branded(f"⚠️ Error: {e}"))
        return

    if file_res.status_code != 200:
        (finish if loading else bot.send_message)(loading or chat_id, branded(f"⚠️ Download failed: {file_res.text[:200]}"))
        return
    ...
    if choice == "video":
        ...
        bot.send_video(chat_id, file_res.content, caption=caption)
        return

    audio_bytes = extract_audio_ffmpeg(file_res.content)
    if not audio_bytes:
        msg = branded("⚠️ Couldn't extract audio from this video (no audio track, or ffmpeg isn't installed on the server).")
        (finish if loading else bot.send_message)(loading or chat_id, msg)
        return

    if loading:
        finish(loading, branded("✅ Sending audio..."))
    bot.send_audio(chat_id, audio_bytes, caption=caption, title=info.get("title"), performer=info.get("author"))
...
def youtube_download(message):
    ...
    url = parts[1].strip()
    loading = send_loading(message, "Fetching YouTube video")
    ask_video_or_audio(loading, "youtube", url, "this video")
...
def deliver_youtube(chat_id, url, choice, loading=None):
    try:
        info = api_post(YOUTUBE_API, "/api/info", {"url": url}).json()
    except Exception:
        info = {}
    ...
    if loading:
        finish(loading, branded("✅ Sending audio..."))
    bot.send_audio(chat_id, file_res.content, caption=yt_caption(info), title=info.get("title"), performer=info.get("channel"))
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("dltype:"))
def on_dltype_choice(call):
    register(call)
    _, token, choice = call.data.split(":", 2)
    entry = pending_dl.pop(token, None)
    if not entry:
        bot.answer_callback_query(call.id, "⚠️ Expired, send the link again.")
        return

    platform = entry["platform"]
    url = entry["url"]
    chat_id = call.message.chat.id
    feature = f"{platform}_{'audio' if choice == 'audio' else 'download'}"

    allowed, used, limit = check_and_use(data, call.from_user.id, feature)
    if not allowed:
        bot.answer_callback_query(call.id, "🚫 Daily limit reached.")
        bot.send_message(chat_id, limit_reached_text(feature.replace("_", " ").title(), used, limit))
        return

    bot.answer_callback_query(call.id, f"⏳ Fetching {choice}...")
    try:
        bot.edit_message_text(
            branded(f"⏳ {i('Fetching ' + choice)}..."),
    ...
    class _Loading:
        """Lets deliver_* reuse finish()/edit-message on this callback's
        message the same way a normal reply_to() loading message would."""
        def __init__(self, chat_id, message_id):
            self.chat = _Chat(chat_id)
            self.message_id = message_id

    loading = _Loading(chat_id, call.message.message_id)

    if platform == "tiktok":
        deliver_tiktok(chat_id, url, choice, loading)
    elif platform == "youtube":
        deliver_youtube(chat_id, url, choice, loading)
    elif platform == "instagram":
        deliver_instagram(chat_id, url, choice, loading)
    elif platform == "pinterest":
        deliver_pinterest(chat_id, url, choice, loading)
...
def pinterest_download(message):
    ...
    ask_video_or_audio(loading, "pinterest", url, "this pin")


def deliver_pinterest(chat_id, url, choice, loading=None):
    try:
    ...
    if choice == "video":
        ...
        if is_video:
            bot.send_video(chat_id, file_res.content)
        else:
            bot.send_photo(chat_id, file_res.content)
        return

    # audio choice
    if not is_video:
        msg = branded("⚠️ This pin is an image — no audio to extract.")
        (finish if loading else bot.send_message)(loading or chat_id, msg)
        return

    audio_bytes = extract_audio_ffmpeg(file_res.content)
    if not audio_bytes:
        msg = branded("⚠️ Couldn't extract audio (no audio track, or ffmpeg isn't installed on the server).")
        (finish if loading else bot.send_message)(loading or chat_id, msg)
        return

    if loading:
        finish(loading, branded("✅ Sending audio..."))
    bot.send_audio(chat_id, audio_bytes)
...
def instagram_download(message):
    ...
    ask_video_or_audio(loading, "instagram", url, "this post")


def deliver_instagram(chat_id, url, choice, loading=None):
    try:
        ...
        return
    if res.status_code != 200:
        (finish if loading else bot.send_message)(loading or chat_id, branded(f"⚠️ Failed: {res.text[:200]}"))
        return

    result = res.json()
    media = result.get("media", [])
    if not media:
        (finish if loading else bot.send_message)(loading or chat_id, branded("❌ No media found."))
        return
    ...
    if choice == "video":
        ...
        for i_ in range(0, len(media), 10):
            ...
            try:
                ...
                bot.send_message(chat_id, branded(f"⚠️ Batch failed: {e}"))
        return

    # audio choice — pull the first video item's audio track
    video_items = [m for m in media if m["type"] == "video"]
    if not video_items:
        msg = branded("⚠️ This post has no video (photo-only) — no audio to extract.")
        (finish if loading else bot.send_message)(loading or chat_id, msg)
        return

    if loading:
        finish(loading, branded("⏳ Extracting audio..."))

    try:
        video_bytes = requests.get(video_items[0]["url"], timeout=60).content
    except Exception as e:
        bot.send_message(chat_id, branded(f"⚠️ Couldn't fetch video: {e}"))
        return

    audio_bytes = extract_audio_ffmpeg(video_bytes)
    if not audio_bytes:
        bot.send_message(chat_id, branded("⚠️ Couldn't extract audio (no audio track, or ffmpeg isn't installed on the server)."))
        return

    note = "" if len(video_items) == 1 else f"\n\n({len(video_items)} videos in this post — sent audio from the first one)"
    bot.send_audio(chat_id, audio_bytes, caption=branded(quote(f"👤 {result.get('owner') or ''}{note}")))
...
# ══════════════════════════════════════════════════════════════ Auto-audio (any link, no command)

@bot.message_handler(func=lambda m: (
    m.content_type == "text"
    and not (m.text or "").startswith("/")
    and m.chat.id not in awaiting_feature_input
    and detect_link(m.text)[0] is not None
))
def auto_audio_from_link(message):
    register(message)
    platform, url = detect_link(message.text)

    feature = f"{platform}_audio"
    allowed, used, limit = check_and_use(data, message.from_user.id, feature)
    if not allowed:
        bot.reply_to(message, limit_reached_text(feature.replace("_", " ").title(), used, limit))
        return

    loading = send_loading(message, f"Fetching {platform.title()} audio")

    if platform == "tiktok":
        deliver_tiktok(message.chat.id, url, "audio", loading)
    elif platform == "youtube":
        deliver_youtube(message.chat.id, url, "audio", loading)
    elif platform == "instagram":
        deliver_instagram(message.chat.id, url, "audio", loading)
    elif platform == "pinterest":
        deliver_pinterest(message.chat.id, url, "audio", loading)
...
def admin_panel_text():
    ...
    return branded(
        header("Admin Panel", "🛠")
        + quote(f"👥 {b('Users')}: {code(total_users)}   💎 {b('Premium')}: {code(premium_users)}")
        + "\n" + i("Manage users, premium & broadcasts below.")
    )
...
def admin_panel(message):
    ...
    bot.reply_to(message, admin_panel_text(), reply_markup=admin_panel_markup())
    ...
    if action == "stats":
        ...
        bot.send_message(
            ...
            branded(
                header("Bot Stats", "📊")
                + quote(
                    f"👥 {b('Total Users')}   {code(total_users)}\n"
                    f"💎 {b('Premium')}       {code(premium_users)}\n"
                    f"🆓 {b('Free')}          {code(free_users)}\n"
                    f"🛠 {b('Admins')}        {code(total_admins)}"
                )
            ),
        ...
        lines = [header(f"Users — {len(data['users'])} total", "👥")]
        for uid, u in list(data["users"].items())[:30]:
            name = u.get("username") or u.get("first_name") or "—"
            status = "💎" if is_premium(u) else "🆓"
            lines.append(f"{status}  {code(uid)}  {i(name)}")
        remaining = len(data["users"]) - 30
        if remaining > 0:
            lines.append(f"\n{i(f'...and {remaining} more')}")
